...
class CalendarUpdateTask extends Simulation {
       val calendarDataset = csv("taskUpdate.csv")
	   val httpProtocol = http
		.baseURL("http://localhost:7070")
		
	

	val scn = scenario("CalendarUpdateTask")
	...
	.exec(http("Load Event") 
			...
			.check(regex("""UITaskForm.*portal:componentId=([^&]+)(&amp;|&)interactionstate=([^&]+)(&amp;|&).*""").saveAs("saveEvent_componentId"))
			.check(regex("""UITaskForm.*interactionstate=([^&]+)(&amp;|&).*""").saveAs("saveEvent_interactionstate")))
